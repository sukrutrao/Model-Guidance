...
__all__ = ["BcosInception3", "inception_v3", "InceptionOutputs", "_InceptionOutputs"]
...
def inception_v3(
    ...
    """
    assert (
        "norm_layer" in kwargs and "conv_layer" in kwargs
    ), "Requires conv and norm layer classes!"
    BasicConv2d.set_norm_and_conv(
        norm_layer=kwargs["norm_layer"], conv_layer=kwargs["conv_layer"]
    )
    ...
    return BcosInception3(**kwargs)


class BcosInception3(BcosModelBase):
    def __init__(
        self,
        ...
        super(BcosInception3, self).__init__()
        ...
        self.fc = conv_layer(
            2048, num_classes, kernel_size=1, stride=1, padding=0, scale_fact=200
        )
...
class InceptionAux(nn.Module):
    def __init__(
        ...
        self.fc = conv_layer(
            768, num_classes, kernel_size=1, stride=1, padding=0, scale_fact=200
        )
        # Diff End
...
class BasicConv2d(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, **kwargs: Any) -> None:
        ...
        # Diff to torchvision: conv -> proj-conv
        ...
        assert (
            self.conv_layer is not None and self.norm_layer is not None
        ), "Please set conv and norm layer classes before instantiation!"

        self.conv = type(self).conv_layer(
            in_channels, out_channels, bias=False, scale_fact=200, **kwargs
        )
        self.bn = type(self).norm_layer(out_channels, eps=0.001)
    ...
    @classmethod
    def set_norm_and_conv(
        cls,
        *,
        norm_layer: Callable[..., nn.Module] = None,
        conv_layer: Callable[..., nn.Module] = None,
    ):
        cls.norm_layer = norm_layer
        cls.conv_layer = conv_layer
