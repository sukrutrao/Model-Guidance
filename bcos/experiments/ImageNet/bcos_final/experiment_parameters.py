...
# TODO: add 101 from prev experiment
...
resnexts = {
    ...
    for (depth_and_width, batch_size) in zip(
        RESNEXT_DEPTHS_AND_WIDTHS, [
...
# some other minor experiments
other_exps = {
    f"resnet_{depth}-bias=1e-3-offlabel=1e-5": update_default(
        dict(
            model=dict(
                name=f"resnet{depth}"
                # logit_bias= already right
            ),
            criterion=UniformOffLabelsBCEWithLogitsLoss(off_label=1e-5),
        )
    )
    for depth in RESNET_DEPTHS
}
