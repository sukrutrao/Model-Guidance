...
__all__ = [
    ...
    "InterpolatedTargetsWeightedBCEWithLogitsLoss",
]
...
class InterpolatedTargetsWeightedBCEWithLogitsLoss(nn.Module):
    def __init__(self, reduction: str = "mean", off_label_smoothing: bool = False):
        super().__init__()
        self.reduction = reduction
        self.off_label_smoothing = off_label_smoothing

    def forward(self, x: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        assert (
            x.shape == target.shape
        ), f"input ({x.shape}) target ({target.shape}) shape mismatch!"

        # find where to make changes
        is_zero = target.eq(0.0)

        # if off labels should be "smoothened" then do so
        off_label = 0.0
        if self.off_label_smoothing:
            off_label = 1 / x.shape[1]  # 1/num_classes

        # set non-zero to 1. for actual targets
        new_targets = torch.where(is_zero, off_label, 1.0)

        # set zero targets to 1. and keep old targets as weights
        weights = torch.where(is_zero, 1.0, target)

        # calculate loss with new targets and "dynamic" weights
        loss = F.binary_cross_entropy_with_logits(
            x, new_targets, reduction=self.reduction, weight=weights
        )
    ...
    def extra_repr(self) -> str:
        result = f"reduction={self.reduction}, "
        if self.off_label_smoothing:
            result += f"off_label_smoothing={self.off_label_smoothing}, "
        result = result[:-2]
        return result
